...
const EmailOtpVerification = ({ onVerified, prefilledEmail }) => {
  ...
  const handleSendOtp = async () => {
    ...
    try {
      setIsLoading(true);
      const res = await axios.post(`http://localhost:8000/api/forgot-password/send-otp`, { email });
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      setIsLoading(true);
      const res = await axios.post(`http://localhost:8000/api/forgot-password/verify-otp`, { email, otp });
